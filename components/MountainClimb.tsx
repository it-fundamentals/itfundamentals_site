...
export default function MountainClimb({ dict }: MountainClimbProps) {
  ...
  const pathRef = useRef(null as SVGPathElement | null);
  ...
  const [pathPoint, setPathPoint] = useState({ x: 120, y: 830 } as Point);
  const [milestones, setMilestones] = useState([] as Point[]);
              ...
              >
                <span
                  className={`w-8 h-8 rounded-lg flex items-center justify-center font-extrabold text-sm ${
                    activeStageId === stage.id ? 'bg-black/5' : 'bg-white/5'
                  }`}
                >
                  {idx + 1}
                </span>
      ...
      <aside className="sticky top-32 right-0 h-[calc(100vh-160px)] w-80 flex-shrink-0 hidden lg:block z-[60]">
        <div className="h-full bg-[#0a0e18]/40 backdrop-blur-2xl border border-white/10 rounded-[32px] p-6 relative overflow-hidden shadow-2xl">
          ...
          <svg className="w-full h-full" viewBox="0 0 240 860" preserveAspectRatio="none">
            ...
            {milestones.map((m: Point, i: number) => (
              <circle
                key={i}
                cx={m.x}
                cy={m.y}
...
function StageCard({ stage, onInView }: StageCardProps) {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-5xl md:text-7xl font-black tracking-tighter mb-10 leading-[0.9] text-white">{stage.title}</h2>
      ...
      <div className="flex gap-5">
        {stage.icons.map((icon: string, i: number) => (
          <div
            key={i}
            title={stage.iconLabels[i]}
            className="w-16 h-16 bg-white/5 border border-white/10 rounded-[20px] flex items-center justify-center text-3xl hover:bg-white/10 hover:scale-110 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-all duration-500"
          >
            {icon}
          </div>
        ))}
