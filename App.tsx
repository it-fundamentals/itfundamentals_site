...
export default function App() {
  ...
  const dict = useMemo(() => I18N[lang as Language], [lang]);
              ...
              {LANGUAGES.map((l) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all ${
                    (lang as Language) === l.code ? 'bg-white text-[#0b1020]' : 'bg-white/5 text-white hover:bg-white/10'
                  }`}
                  type="button"
                >
        ...
        <main className="relative z-10">
          <AnimatePresence mode="wait">
            <Routes>
              ...
              <Route
                path="/about"
                element={<PageTemplate title={dict.page_about_title} body={dict.page_about_body} dict={dict} />}
              />
              <Route
                path="/contact"
                element={<PageTemplate title={dict.page_contact_title} body={dict.page_contact_body} dict={dict} />}
              />
              <Route
                path="/portal"
                element={<PageTemplate title={dict.page_portal_title} body={dict.page_portal_body} dict={dict} />}
              />
            </Routes>
          </AnimatePresence>
        </main>
